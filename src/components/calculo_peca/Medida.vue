...
export default {
  ...
  methods: {
    ...
    conversao_polegada() {
      this.medida.valor = Math.round((this.pol * (this.polint + this.polnum / this.polden)) * 1000) / 1000
      this.medida.polegada = ''
      if (this.polint) this.medida.polegada = `${this.polint}`
      if (this.polnum) this.medida.polegada += ((this.polint) ? `.` : '') + `${this.polnum}/${this.polden}`
    },
  },
};
</script>
